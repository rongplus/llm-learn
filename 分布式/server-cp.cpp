...
void init_mysql_pool() {
    for (int i = 0; i < MYSQL_POOL_SIZE; ++i) {
        ...
        if (!mysql_real_connect(conn, "192.168.1.105", "root", "123456", "employees", 0, nullptr, 0)) {
...
void init_redis_pool() {
    for (int i = 0; i < REDIS_POOL_SIZE; ++i) {
        redisContext *conn = redisConnect("192.168.1.105", 6379);
...
void handle_client(int client_sock) {
    ...
    while (true) {
        ...
        if (bytes_received <= 0) break; // 客户端断开连接
        buffer[bytes_received] = '\0';
        std::string request(buffer);
        std::string response;

        // 处理 MySQL 请求
        if (request.find("MYSQL:") == 0) {
        ...
        // 处理 Redis 请求
        else if (request.find("REDIS:") == 0) {
            std::string command = request.substr(6);
